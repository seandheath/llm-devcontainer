...
in
{
  ...
  mkDevContainer =
    ...
    let
      ...
      # Project path - use $(pwd) if projectPath is a nix store path (flake context)
      # This allows the mount to work correctly when run from the actual project directory
      ${if lib.hasPrefix "/nix/store/" projectPath then ''
      PROJECT_PATH="$(pwd)"
      '' else ''
      ...
      runScript = pkgs.writeShellScript "llm-devcontainer-run" ''
        ...
        #
        # Project: ${name}
        # Container workspace: /${sanitizedName}
        #
        # Usage:
